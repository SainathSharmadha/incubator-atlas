...
args=str(sys.argv)
totalreg=int((sys.argv[1]))
file_path=sys.argv[2]
stat_file=open('atlas_stat_ctas.txt','w')
class Table:
...
tables=[]
for table_num in range(0,totalreg+1) :
...
for lines in content:
	...
	if start_match:
		qualified_name=start_match.group(7)
	   	table_name_match=re.match('default\.table_([0-9]*)\_ctas@erie-perf-test-cluster',qualified_name)
	   	if table_name_match:
...
for table_num in range(0,totalreg+1):
        if tables[table_num].getTableName() :
